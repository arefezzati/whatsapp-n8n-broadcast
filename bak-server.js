...
app.get("/api/whatsapp-contacts", async (req, res) => {
  ...
  try {
    ...
    // Detaylı filtreleme (grupları ve kendini çıkar)
    const filteredContacts = allContacts.filter(contact => {
      ...
      const isNotMe = !contact.id.includes(':'); // Kendim değilim
      const hasName = contact.name || contact.pushname || contact.verifiedName || contact.notify;
      const hasValidPhone = contact.id.includes('@s.whatsapp.net');

      return isNotGroup && isNotBroadcast && isNotMe && hasName && hasValidPhone;
    });
...
// N8N → YENİ: Batch sistemi - N video aynı hedefe (grup veya kişi), sonra diğer hedefe (fanout ile)
/*
app.post("/send-video-to-contacts-grouped", async (req, res) => {
  ...
  try {
    ...
    // 3) DİĞER HEDEFLERE FORWARD (Baileys forward API)
    let forwardedCount = 0;
    logger.info(`[GROUPED-FORWARD] ${unifiedTargets.length - 1} hedefe forward başlıyor...`);

    for (let i = 1; i < unifiedTargets.length; i++) {
      const target = unifiedTargets[i];

      logger.info(`[FORWARD] ${i}/${unifiedTargets.length - 1}: ${target.name}`);
      ...
      try {
        for (const msg of sentMessages) {
          ...
          logger.debug(`[FORWARD] Message forwarded in ${fwdDuration}ms`);
        }
        ...
        // Burst control: Her forward sonrası delay
        await delay(FORWARD_DELAY);

        // Burst control: 30 kişiye ulaşınca 60s bekle
        if (forwardedCount % FORWARD_BURST_SIZE === 0) {
          logger.info(`[GROUPED-FORWARD] ${forwardedCount} forward tamamlandı, ${FORWARD_BURST_COOLDOWN / 1000}s cooldown...`);
          addActivityLog('info', `${forwardedCount} kişiye gönderildi, ${FORWARD_BURST_COOLDOWN / 1000}s bekleniyor...`);
        ...
        // Devam et, tüm hedefleri atlamayalım
      }
    }

    // 4) Cache temizle
    clearAllCache();
    logger.info(`[GROUPED-FORWARD] Tüm işlemler tamamlandı: ${videoUrls.length} video → ${unifiedTargets.length} hedef (1 send + ${forwardedCount} forward)`);
    addActivityLog('success', `Tamamlandı: ${unifiedTargets.length} hedef, ${videoUrls.length} video`);
    ...
    return res.status(500).json({ error: "Video işlem hatası: " + error.message });
  }
});
*/
...
app.post("/send-video-to-contacts-grouped", async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < videoUrls.length; i++) {
      ...
      logger.info(`[SEND] Video ${i + 1}/${videoUrls.length} cache'leniyor...`);
      const cacheStart = Date.now();
      const cachedPath = await getOrCacheVideo(vUrl);  // Zaten async varsayıyorum
      const cacheTime = Date.now() - cacheStart;
      
      // Async read için (sync yerine)
    ...
    for (let i = 1; i < unifiedTargets.length; i++) {
      ...
      try {
        for (let v = 0; v < sentMessages.length; v++) {  // Video indeksi ekle
          ...
          // İYİLEŞTİRME: Her forward (video) arası delay + rastgelelik
          const randomDelay = FORWARD_DELAY + Math.random() * 200;  // 600ms + 0-200ms rastgele
        ...
        // Burst control: Her forward sonrası ekstra delay (hedef tamamlandıktan sonra)
        await delay(FORWARD_DELAY);  // Hedef arası ekstra
    ...
    const processingTime = Date.now() - startTime;
    // İYİLEŞTİRME: Estimated time hesapla (burst dahil)
    const estTimeMs = (forwardedCount * (sentMessages.length * FORWARD_DELAY + FORWARD_DELAY)) + (Math.floor(forwardedCount / FORWARD_BURST_SIZE) * FORWARD_BURST_COOLDOWN);
    const estMinutes = Math.ceil(estTimeMs / 1000 / 60);
    ...
    const responseData = {
      ...
      forwardedTo: forwardedCount,
      processingTime,
      estimatedTime: `~${estMinutes} dakika`
    };
